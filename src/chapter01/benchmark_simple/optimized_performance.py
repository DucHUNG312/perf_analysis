...
from chapter01.benchmark_simple.workload_config import WORKLOAD
...
try:
    import chapter01.arch_config  # noqa: F401 - Apply chapter defaults
except ImportError:
    pass
...
class OptimizedPerformanceBatchBenchmark(VerificationPayloadMixin, BaseBenchmark):
    ...
    def __init__(self, batch_size: int = 32):
        ...
        samples = float(self.batch_size *
                        self.workload.performance_microbatches)
    ...
    def setup(self) -> None:
        ...
        microbatches = [
            torch.randn(self.batch_size, self.hidden_dim,
                        device=self.device, dtype=dtype).contiguous()
        ...
        for start in range(0, len(self.microbatches), self.fusion):
            batch = torch.cat(
                self.microbatches[start: start + self.fusion], dim=0)
    ...
    def capture_verification_payload(self) -> None:
        ...
        with torch.no_grad():
            ...
            if model_params:
                verify_input = verify_input.to(
                    dtype=model_params[0].dtype, device=self.device)
