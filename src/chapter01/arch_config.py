...
try:
    ...
    _filter_thread = threading.Thread(
        target=_filter_stderr_thread, daemon=True)
...
warnings.filterwarnings(
    "ignore", message=".*Please use the new API settings to control TF32.*", category=UserWarning)
# CUDA capability 12.1 warning - GPU is newer than officially supported but works fine
warnings.filterwarnings(
    "ignore", message=".*Found GPU.*which is of cuda capability.*", category=UserWarning)
warnings.filterwarnings(
    "ignore", message=".*Minimum and Maximum cuda capability supported.*", category=UserWarning)
