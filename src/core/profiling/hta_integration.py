...
class HTAAnalyzer:
        ...
        try:
            ...
            try:
                ...
                if temporal is not None and not temporal.empty:
                    ...
                    if total_time > 0:
                        report.compute_time_pct = temporal.get(
                            'compute', temporal.sum(axis=1)).sum() / total_time * 100
                        report.gpu_idle_time_pct = temporal.get('idle', 0).sum(
                        ) / total_time * 100 if 'idle' in temporal else 0
                        report.communication_time_pct = temporal.get('communication', 0).sum(
                        ) / total_time * 100 if 'communication' in temporal else 0
            except Exception:
                pass
            ...
            try:
                ...
                if idle_time is not None:
                    report.bottlenecks.append(
                        f"GPU idle time: {report.gpu_idle_time_pct:.1f}%")
            except Exception:
                pass
            ...
            try:
                ...
                if comm is not None:
                    report.raw_analysis['comm_overlap'] = comm.to_dict(
                    ) if hasattr(comm, 'to_dict') else str(comm)
            except Exception:
                pass
    ...
    def _manual_trace_analysis(self, trace_path: Path) -> HTAReport:
        ...
        try:
            ...
            events = trace if isinstance(
                trace, list) else trace.get('traceEvents', [])
            ...
            if total_time > 0:
                ...
                report.gpu_idle_time_pct = max(
                    0, 100 - report.compute_time_pct - report.communication_time_pct)
    ...
    def _generate_recommendations(self, report: HTAReport) -> List[str]:
        ...
        if report.top_kernels:
            ...
            total_kernel_time = sum(k['duration_us']
        ...
        if not recommendations:
            recommendations.append(
                "Profile looks well-optimized. Consider micro-optimizations if needed.")
